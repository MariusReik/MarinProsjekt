...
- 2026-07-16: Issue #15 (gap-varsling, uke 6 steg 2). Kanal = **webhook først** (HTTP POST JSON via Spring `RestClient`, ingen ny dependency/infra, testbar mot mock-server); e-post (`spring-boot-starter-mail` + SMTP) utsatt til eget oppfølgings-issue. Interface `GapAlertNotifier` gjør e-post til et drop-in senere. Trigges av **egen `@Scheduled` polling-jobb** (`GapAlertService`), ikke piggyback på deteksjonssyklusen: den poller `gap_events WHERE alerted_at IS NULL`, sender, og stempler `alerted_at` — frikoblet fra deteksjon (som #8) og gir gratis retry (feilet levering lar `alerted_at` stå NULL → plukkes neste syklus). Dedup + delivery-logging = ny nullbar `alerted_at`-kolonne (Flyway `V4__gap_alert.sql`, partial index `WHERE alerted_at IS NULL`) pluss strukturert SLF4J-logg per levering; ingen egen delivery-tabell (mer enn minste steg). Per-gap feil logges og hoppes over (batch fortsetter), hele syklusen wrappes så scheduler ikke kanselleres. Config i `app.alerting.*` (`webhook-url` tom = av, `check-interval-ms` 60000, `batch-limit` 50). Fullfører uke 6-DoD-en (anomali #1 i produksjon m/varsling). Java ikke kompilert i sandkassen — verifiseres av CI/lokal `mvn test`.
